...
This is the RFM69Pi firmware from Open Energy Monitor modified to run at 115200 buad instead of 38400.
In this LoRa Bridge project it is used on a non-USB Moteino attached to a LoRa Moteino via TX, GND, and VCC.

After upload send the following commands via serial (for example). 
14i to set node to 14, or whatever node is available in your network.
210g to join the network group 210, or whatever node group your network is using. 

210 is the default for Open Energy Monitor, and 14 is often available.
*/
...
#define SERIAL_BAUD 115200   // max baud for 8Mhz RFM12Pi http://openenergymonitor.org/emon/node/6244/
